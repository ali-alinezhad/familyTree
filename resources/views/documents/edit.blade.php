    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-6 pr-5">
                {{ Form::open(['route'=>['document.update',$locale,$document->id], 'method' => 'put', 'enctype'=>"multipart/form-data"]) }}
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="ckeditor form-control" name="description"
                                  id="my_ckeditor">{{ $document->description }}</textarea>
                        @error('description')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="checkbox" name="status" id="status" @if($document->status) checked @endif>
                        @error('status')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="col-md-6">
                @if(strpos($document->file,'.pdf') !== false)
                    <embed src="{{ asset($document->file) }}" width="100%" height="80%">
                @else
